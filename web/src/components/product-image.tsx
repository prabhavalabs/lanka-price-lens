...
/**
 * Product photo served from `data/images/products/<slug>.jpg`, served by the API at `/images/products/`. Falls back to a
...
export function ProductImage({ id, label, size = "md", className }: { id: string; label: string; size?: keyof typeof sizes; className?: string | undefined }) {
  ...
  return <img alt="" className={cn("shrink-0 bg-muted object-cover ring-1 ring-white/10", sizes[size], className)} decoding="async" loading="eager" onError={() => setFailedId(id)} src={`/images/products/${slug}.jpg`} />;
}
