...
import { telegramMessageOf } from "./telegram.ts";
...
export type NewsletterDeps = {
  ...
  /** Linked Telegram chats get the same mail as a message. The public channel's digest and the platforms' posts are jobs of their own (api/src/social/jobs.ts). */
  telegram?: { store: TelegramStore } | undefined;
  now?: (() => Date) | undefined;
  ...
  const runNewsletter = async (kind: NewsletterKind, options: RunOptions): Promise<RunOutcome> => {
    ...
    try {
      ...
      if (kind === "deals_daily") {
        ...
        // The channel's digest and the platforms' posts are jobs of their own now, each on its own
        // recurrence (api/src/social/jobs.ts). This run is the mail, and nothing else: two owners
        // for one post is how a day ends up posted twice.
