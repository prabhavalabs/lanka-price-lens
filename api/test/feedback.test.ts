...
test("visitors can send feedback within a budget and the owner works through it in the admin", async () => {
  ...
  const sent: Array<{ title: string; summary?: string | undefined }> = [];
  try {
    const app = createApp(database, undefined, undefined, { ownerNotifier: { configured: true, targets: ["test"], notify: async (message) => { sent.push(message); return []; } } });
    ...
    assert.match(sent[0]!.title, /^\[PriceLens\] Bug report: The chart does not load/u);
