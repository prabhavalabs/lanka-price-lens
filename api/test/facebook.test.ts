...
import { dealsCardSvg, facebookDealsPost, postCardHeight, postCardWidth, postDeals, renderDealsCard } from "../src/facebook/post.ts";
import { facebookStateCookie, readConnectState, signConnectState } from "../src/facebook/routes.ts";
import { openToken, sealToken } from "../src/facebook/seal.ts";
import { createFacebookStore } from "../src/facebook/store.ts";
...
test("the store keeps tokens sealed, one Page active, and a dead token out of the channel's reach", () => {
  ...
  try {
    const store = createFacebookStore(database, "state-secret");
    const pages = store.connect([page("555000111", "Read only", false), page("1234567890", "PriceLens")], "Nipun", now);
    assert.deepEqual(pages.map((entry) => [entry.name, entry.active, entry.can_post]), [["PriceLens", true, true], ["Read only", false, false]]);
    assert.equal(store.tokenFor("1234567890"), "token-of-1234567890");
    const raw = database.prepare("SELECT token_sealed FROM facebook_page WHERE page_id = '1234567890'").get() as { token_sealed: string };
    assert.ok(!raw.token_sealed.includes("token-of"), "the database never holds the token in the clear");
    assert.equal(createFacebookStore(database, "rotated-secret").tokenFor("1234567890"), null);

    assert.equal(store.activate("555000111", now), true);
    assert.equal(store.active()?.page_id, "555000111");
    assert.equal(store.list().filter((entry) => entry.active).length, 1);
    assert.equal(store.activate("unknown", now), false);

    store.setPaused("1234567890", true, now);
    store.markToken("1234567890", { valid: false, error: "FACEBOOK_190: session invalidated" }, now);
    assert.equal(store.tokenFor("1234567890"), null);
    const dead = store.list().find((entry) => entry.page_id === "1234567890")!;
    ...
    store.connect([page("1234567890", "PriceLens Sri Lanka")], "Nipun", new Date(now.getTime() + 60_000));
    const back = store.list().find((entry) => entry.page_id === "1234567890")!;
    assert.deepEqual([back.name, back.token_status, back.paused, back.active], ["PriceLens Sri Lanka", "ok", true, false]);
    assert.equal(store.tokenFor("1234567890"), "token-of-1234567890");
    assert.equal(store.disconnect("1234567890"), 1);
    assert.equal(store.disconnect(null), 1);
...
test("the admin connects a Page through Facebook Login, previews the day, and posts it", async () => {
  ...
  try {
    ...
    assert.equal((await app.request("/v1/admin/facebook", { headers: json })).status, 401);
    ...
    const empty = (await (await app.request("/v1/admin/facebook", { headers: { ...json, cookie: admin } })).json()) as { payload: { configured: boolean; redirect_uri: string; pages: unknown[] } };
    assert.deepEqual([empty.payload.configured, empty.payload.redirect_uri, empty.payload.pages.length], [true, "http://localhost/v1/admin/facebook/callback", 0]);

    // Connect: the admin session opens the consent screen and leaves the signed state behind.
    assert.equal((await app.request("/v1/admin/facebook/connect", { headers: { host: "localhost" } })).status, 401);
    const connect = await app.request("/v1/admin/facebook/connect", { headers: { host: "localhost", cookie: admin } });
    ...
    assert.equal(forged.headers.get("location"), "/admin/facebook?facebook=expired");
    ...
    assert.equal(mismatched.headers.get("location"), "/admin/facebook?facebook=expired");
    ...
    assert.equal(cancelled.headers.get("location"), "/admin/facebook?facebook=cancelled");
    ...
    assert.equal(refused.headers.get("location"), "/admin/facebook?facebook=failed");
    ...
    assert.equal(connected.headers.get("location"), "/admin/facebook?facebook=connected");

    const status = (await (await app.request("/v1/admin/facebook", { headers: { ...json, cookie: admin } })).json()) as { payload: { pages: Array<Record<string, unknown>> } };
    assert.equal(status.payload.pages.length, 1);
    assert.deepEqual([status.payload.pages[0]!.name, status.payload.pages[0]!.active, status.payload.pages[0]!.connected_by], ["PriceLens", true, "Nipun"]);
    assert.ok(!JSON.stringify(status.payload).includes("PAGE-TOKEN"), "the token never leaves the server");

    const preview = (await (await app.request("/v1/admin/facebook/preview?day=2026-09-20", { headers: { ...json, cookie: admin } })).json()) as { payload: { ready: boolean; caption: string; image_url: string } };
    ...
    const check = (await (await app.request("/v1/admin/facebook/check", { method: "POST", headers: { ...json, cookie: admin } })).json()) as { message: string };
    assert.equal(check.message, "Facebook accepts the token");

    const posted = (await (await app.request("/v1/admin/facebook/post", { method: "POST", headers: { ...json, cookie: admin }, body: JSON.stringify({ day: "2026-09-20" }) })).json()) as { message: string; payload: { post: { status: string; url: string }; posts: unknown[] } };
    assert.equal(posted.message, "Posted to the Page");
    ...
    assert.equal((await app.request("/v1/admin/facebook/pages/1234567890", { method: "DELETE", headers: { ...json, cookie: admin } })).status, 200);
    assert.equal((await app.request("/v1/admin/facebook/post", { method: "POST", headers: { ...json, cookie: admin }, body: "{}" })).status, 409);
  } finally {
    database.close();
  }
});
